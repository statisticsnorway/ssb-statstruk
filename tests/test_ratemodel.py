...
def test_statstruk_ratemodel_1obs(capfd):
    ...
    mod1.fit(x_var="employees", y_var="job_vacancies", strata_var="industry", control_extremes=True)
    
    # Use capfd to capture the print output
    out, err = capfd.readouterr()
    
    # Assert that the captured output matches the expected message
    assert out == "Stratum: 'G', has only one observation and has 0 variance. Consider combing strata.\n"
...
def test_stastruk_ratemodel_check_neg() -> None:
    ...
    with pytest.raises(ValueError) as error_mes:
        ...
        mod1.fit(x_var="employees", y_var="job_vacancies", strata_var="industry", control_extremes=False)
    assert "There are negative values in the variable 'employees' in the sample dataset. Consider a log transformation or another type of model." in str(error_mes.value)
...
def test_stastruk_ratemodel_check_sample0(capfd) -> None:
    ...
    # Assert that the captured output matches the expected message
    assert out == "Values of zero detected in stratum 'B'. These observations will not be assessed for extremeness.\n"
...
def test_stastruk_ratemodel_check_allbutone0(capfd) -> None:
    ...
    sample1 = sample1.loc[sample1.job_vacancies.notna(), ]
    pop1 = pd.read_csv(pop1_file)
    new_rows = pd.DataFrame({'id':[10006,10007], 'employees':[50,50], 'employees_f': [20,25], 
                         'employees_m': [30, 25], 'turnover':  [15000,15000], 'size' : ["mid","mid"], 'industry': ['G','G']})
    pop1 = pd.concat([pop1,new_rows], ignore_index=True)

    new_rows2 = pd.DataFrame({'id':[10006,10007], 'employees':[50,50], 'employees_f': [20,25], 
                         'employees_m': [30, 25], 'turnover':  [15000,15000], 'size' : ["mid","mid"], 'industry': ['G','G'],
                         'job_vacancies':[0,0], 'sick_days':[70,65], 'sick_days_f':[35, 25], 'sick_days_m':[35,40]})
    ...
    # Assert that the captured output matches the expected message
    assert out == "Only one non-zero value found for 'job_vacancies' in stratum 'G'. Extreme detection can't be performed for the non-zero observation.\n"
...
def test_stastruk_ratemodel_check_all0(capfd) -> None:
    ...
    # Assert that the captured output matches the expected message
    assert out == "All values for 'job_vacancies' in stratum 'G' were zero. Extreme values need to be checked in other ways for this stratum.\n"
