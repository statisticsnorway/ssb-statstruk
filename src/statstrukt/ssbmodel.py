...
class ssbmodel:
    ...
    def __init__(self, pop_data: pd.DataFrame, sample_data: pd.DataFrame, id_nr: str) -> None:
        ...
        self._check_variable(id_nr, self.pop_data, data_name = "population", check_for_char=True, remove_missing=False)
        self._check_variable(id_nr, self.sample_data, check_for_char=True, remove_missing=False)
        self.id_nr = id_nr
    
    def __call__(self):
        print("strukturmodel instance with data including population size " + self.pop.shape[0] + " and sample size " + self.sample_data.shape[0])
    
    def _check_variable(self, var_name, dataset, data_name = "sample", check_for_char=False, remove_missing = False):
        """
        ...
        if var_name not in dataset:
            raise ValueError(f"Variable '{var_name}' not found in the {data_name} dataset.")
        ...
        if not np.issubdtype(value.dtype, np.number):
            raise ValueError(f"Variable '{var_name}' in the {data_name} dataset needs to be numeric but isn't.")
        ...
        if remove_missing:
            ...
            if  missing_num > 0 :
                print(f"There were {missing_num} missing values in variable {var_name}. These observations were reomved from the sample")
            self.sample_data = cleaned_dataframe
        else:

            # Give error for missing values
            if value is None or (isinstance(value, float) and np.isnan(value)):
                raise ValueError(f"Variable '{var_name}' contains missing values. Please fix and try again.")
    ...
    def _add_flag(self):
        ...
        self.pop_data[self.flag_var] = self.pop_data[self.id_nr].apply(lambda x: 1 if x in sample_ids else 0)
