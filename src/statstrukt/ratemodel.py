...
class ratemodel(ssbmodel):
    ...
    """
    def __init__(self, pop_data: pd.DataFrame, sample_data: pd.DataFrame, id_nr: str) -> None:
        super().__init__(pop_data, sample_data, id_nr)
        self.pop_data = pop_data
        self.sample_data = sample_data
        

    def fit(self, y_var: str, x_var: str, strata_var: str = None, exclude: list[str]= None, exclude_auto: int = 0, quiet: bool = True) -> None:
        """
        ...
        self._check_variable(strata_var, self.pop_data, data_name = "population",check_for_char=True)
        ...
        all_values_present = all(value in unique_strata_sample for value in unique_strata_pop)
        assert all_values_present, "Not all strata in the population were found in the sample data. Please check"
        all_values_present = all(value in unique_strata_pop for value in unique_strata_sample)
        assert all_values_present, "Not all strata in the sample were found in the population data. Please check"
        ...
        def _update_strata(df, exclude):
            ...
            df.loc[mask, '_strata_var_mod'] = df.loc[mask, '_strata_var_mod'] + "_surprise_" + df.loc[mask, self.id_nr].astype(str)
        ...
        for stratum, group in self.sample_data.groupby(self.strata_var_mod):
            if (len(group) > 1):  # Ensure there is more than one row to fit a model
                ...
                sigma2 = (1/(len(group) - 1)) * sum(model.resid.values ** 2 / group[x_var])
                stratum_info = {
                ...
                rstuds = _get_rstud(y=group[y_var], res=model.resid.values, x_var=x_var, df=group, 
                                    hh=hats, X=group[x_var], formula=formula)
    ...
    def _get_domain_estimates(self, domain: str):
        ...
        for d in domain_unique:
            ...
            for s in strata_unique:
                mask_s = (temp_dom[strata_var] == s) & (temp_dom[self.flag_var] == 0) # Those not in sample
    ...
    def _get_domain(self, domain: str) -> pd.DataFrame:
        ...
        one_to_many = {strata_var: domains for strata_var, domains in domain_key.items() if len(set(domains)) > 1}
        assert len(one_to_many) == 0, "1-to-many relationship(s) found in strata/domain aggregation. Only aggregated variances are programmed."
    ...
    def get_estimates(self, domain: str = None, var_type: str = "robust") -> pd.DataFrame:
        """
        ...
        # If domains are not aggregates of strata run alternative calculation for variance (not robust)
            ...
            if (var_type == "robust"):
                print("Domain variable is not an aggregation of strata variables. Only standard variance calculations are available")
        ...
        strata_df[self.y_var + '_est'] = pd.to_numeric(strata_df['beta'] * strata_df['x_sum_pop'])
        ...
        # Aggregate to domain
        result = strata_df[[domain, "N", "n", "job_vacancies_est","var1","var2","var3"]].groupby(domain).sum()
        
        # Add in CV
        result[self.y_var +'_CV1'] = np.sqrt(result.var1) / result[self.y_var + '_est'] * 100 
        result[self.y_var +'_CV2'] = np.sqrt(result.var2) / result[self.y_var + '_est'] * 100  
        result[self.y_var +'_CV3'] = np.sqrt(result.var3) / result[self.y_var + '_est'] * 100  
    ...
    def get_extremes(self, rbound: float = 2, gbound: float = 2) -> pd.DataFrame:
        ...
        extremes = extremes[[self.id_nr, self.strata_var_mod,'n','N', self.x_var, self.y_var,'y_est','y_est_ex', 'gbound', 'G', 'rstud']]
        return extremes
    ...
    def get_imputed(self) -> pd.DataFrame:
        ...
        pop[f'{self.y_var}_imputed'] = pop[self.id_nr].map(id_to_yvar_map).fillna(pop[f'{self.y_var}_imputed'])
